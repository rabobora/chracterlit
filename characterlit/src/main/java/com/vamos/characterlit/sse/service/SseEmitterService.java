...
public class SseEmitterService {
    ...
    public SseEmitter subscribe(String bidId, Long userNumber) {
        SseEmitter emitter = createEmitter();
        log.info("Creating new SseEmitter for bidId: {}", bidId);

        emitter.onCompletion(() -> {
            removeEmitter(bidId, userNumber);
            log.info("Emitter completed for bidId: {}, nickname: {}", bidId, userNumber);
        });
        emitter.onTimeout(() -> {
            log.info("Emitter timeout for bidId: {}, nickname: {}", bidId, userNumber);
        });
        emitter.onError(e -> {
            log.error("Error in emitter for bidId: {}, message: {}", bidId, e.getMessage());
        });
        emitterMap.computeIfAbsent(bidId, k -> new ConcurrentHashMap<>()).put(userNumber, emitter);

        log.info("Added SseEmitter to list for bidId: {}, nickname: {}", bidId, userNumber);
        ...
        try {
            ...
            SseEmitter.SseEventBuilder event = SseEmitter.event()
                    ...
                    .data("SSE connected as: " + userNumber + "present bid" + nowbid.getPresentBid())
                    .reconnectTime(RECONNECTION_TIMEOUT);
            emitter.send(event);
        } catch (IOException e) {
            log.error("Failure sending initial data for bidId: {}, nickname: {}, message: {}", bidId, userNumber, e.getMessage());
        }
        return emitter;
    }
    ...
    public void bidSuccessAlarm(EventPayload eventPayload) {
        SseEmitter bidderEmitter = emitterMap.get("bidId" + eventPayload.bidId()).get(eventPayload.userNumber());
        try {
            bidderEmitter.send(SseEmitter.event()
                    ...
                    .reconnectTime(RECONNECTION_TIMEOUT)
                    .data(eventPayload, MediaType.APPLICATION_JSON));
            log.info("Sent BID SUCCESS notification, bidId={}, userId={}", eventPayload.bidId(), eventPayload.userNumber());

        } catch (Exception e) {
            log.error("Failed to send BID SUCCESS emitter for bidId={}, userId={}, {}", eventPayload.bidId(), eventPayload.userNumber(), e.getMessage());
        }
    }
    private void removeEmitter(String bidId, Long userNumber) {
        Map<Long, SseEmitter> userEmitters = emitterMap.get(bidId);
        if (userEmitters != null) {
            userEmitters.remove(userNumber);
            if (userEmitters.isEmpty()) {
                emitterMap.remove(bidId);
                log.info("Removed last emitter, deleting map for bidId: {}", bidId);
            }
            log.info("Removed emitter for bidId={}, nickname={}", bidId, userNumber);
        }
    }
    ...
    public void disconnect(DisconnectDTO disconnectDTO) {
        log.info("SSE disconnect request Activated bidId={}, userNumber={}", disconnectDTO.getBidId(), disconnectDTO.getUserNumber());
        removeEmitter("bidId" + disconnectDTO.getBidId(), disconnectDTO.getUserNumber());
    }
}
